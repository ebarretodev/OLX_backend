...
//Libs para controle de imagem
const {v4: uuid} = require('uuid')
const jimp = require('jimp')
//função para controle da imagem
...
module.exports = {
    ...
    addAction: async (req, res)=>{
        ...
        if(!title || !cat){
            return res.json({error: "Titulo ou categoria não preenchidos!"})
        }
        ...
        if(cat.length < 12){return res.json({error: 'Categoria não existente!'})}
        const category = await Category.findById(cat).exec()
        if(!category){return res.json({error: 'Categoria não existe!'})}
    ...
    editAction: async (req, res)=>{
        ...
        const user = await User.findOne({token}).exec()
        if(user._id.toString() !== ad.idUser.toString()){return res.json({error:'Este anúncio não é seu!'})}

        let updates = {}

        if(title){updates.title = title}
        if(price){updates.price = parseFloat(price.replace('.', '').replace(',','.').replace('R$ ', ''))}
        if(priceneg){updates.priceNegotiable = priceneg}
        if(status){updates.status = status}
        if(desc){updates.description = desc}
        if(cat){
            const category = await Category.findOne({slug: cat}).exec()
            if(!category){return res.json({error:'Categoria inexistente!'})}
